    ...
    if(index <= trueLengthValue){
        ...
        if (0xD800 <= characterIntegerValue && 0xDBFF >= characterIntegerValue){
            if (substringStartCodePoint < codePointLengthValue){
                unsigned short nextCharacterIntegerValue = [self characterAtIndex:(substringStartCodePoint + 1)];
                if (0xDC00 <= nextCharacterIntegerValue && 0xDFFF >= nextCharacterIntegerValue){
                    resultCharArray[0] = (char)(characterIntegerValue >> 8);
                    resultCharArray[1] = (char)(characterIntegerValue & 0xFF);
                    resultCharArray[2] = (char)(nextCharacterIntegerValue >> 8);
                    resultCharArray[3] = (char)(nextCharacterIntegerValue & 0xFF);
                    resultCharArray[4] = '\0';
                }
                else{
                    resultCharArray[0] = (char)(characterIntegerValue >> 8);
            ...
            return [NSString stringWithCString:resultCharArray encoding:NSUTF16StringEncoding];
        }
        else if(0x0000 <= characterIntegerValue && 0x00FF >= characterIntegerValue){
            // resultCharArray[0] = (char)0x0000;
            resultCharArray[0] = (char)(characterIntegerValue & 0xFF);
            resultCharArray[1] = '\0';
            return [NSString stringWithCString:resultCharArray encoding:NSUTF8StringEncoding];
        }
        else{
            resultCharArray[0] = (char)(characterIntegerValue >> 8);
            resultCharArray[1] = (char)(characterIntegerValue & 0xFF);
